...
const Header5: React.FC<Props> = ({ isReverse }) => {
  const home = FormatMessage({ msgCode: 'sport.subHeader.hd' })
  const draw = FormatMessage({ msgCode: 'sport.subHeader.ad' })
  const away = FormatMessage({ msgCode: 'sport.subHeader.ha' })
  return (
...
const Header4: React.FC<Props> = ({ isReverse }) => {
  const home = FormatMessage({ msgCode: 'sport.subHeader.home' })
  const draw = FormatMessage({ msgCode: 'sport.subHeader.draw' })
  const away = FormatMessage({ msgCode: 'sport.subHeader.away' })
  return (
    <div className={styles.container}>
      <div className={styles.teamName}>{!isReverse ? home : away}</div>
      <div className={styles.teamName}>{draw}</div>
